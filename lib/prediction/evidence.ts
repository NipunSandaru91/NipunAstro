...
export type PredictionEvidence = {
  id:string; topic:PredictionTopic; polarity:EvidencePolarity;
  placement:{graha_id:number;rasi_id:number;bhava:number};
  rule:{code:string;text_si:string};
  qualities:{
    graha:ReturnType<typeof grahaQualitySi>;
...
export function createPlacementEvidence(input:PlacementEvidenceInput):PredictionEvidence{
  assertIntegerRange(input.grahaId,1,9,"graha_id");assertIntegerRange(input.rasiId,1,12,"rasi_id");assertIntegerRange(input.bhava,1,12,"bhava");
  return {id:input.id,topic:input.topic,polarity:input.polarity,
