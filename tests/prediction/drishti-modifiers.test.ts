...
Deno.test("Golden Chart: Jupiter aspects Mars in Gemini with special 9th drishti",()=>{
 const m=drishtiModifiers({targetGrahaId:3,positions:GOLDEN,lagnaRasiId:4});
 const j=m.find(x=>x.source_graha_id===5);
 if(!j)throw new Error("Jupiter aspect to Mars missing");
 if(j.code!=="BENEFIC_DRISHTI"||j.polarity!=="SUPPORTING")throw new Error("Jupiter aspect classification changed");
