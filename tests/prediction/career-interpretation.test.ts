...
Deno.test("career conclusion preserves evidence and timing without probability",()=>{
 const c=buildCareerConclusion({theme,timing,reasoning});
...
Deno.test("Sinhala renderer preserves graha rasi bhava rule chain",()=>{
 const r=renderCareerConclusionSi(buildCareerConclusion({theme,timing,reasoning}));
 for(const value of ["කුජ","මිථුන","12","10 වන භාව අධිපති"]){if(!r.includes(value))throw new Error("reasoning chain lost");}
});
